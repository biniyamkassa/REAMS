...
//        var_dump($translations);
    ...
    public function listCommitteeUserAction()
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            // checking required fields
            foreach(array('name', 'username', 'email', 'country', 'institution','phone',) as $field) {
                if(!isset($post_data[$field]) or empty($post_data[$field])) {
                    $session->getFlashBag()->add('error', $translator->trans("Field '%field%' is required.", array("%field%" => $field)));
                    return $output;
                }
            ...
            ->setSubject("[CMHS-REAMS] " . $translator->trans("Set your password"))
    ...
    public function updateCommitteeUserAction($user_id)
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            if(isset($post_data['status']) && !$user_status) {
                ...
                $message = \Swift_Message::newInstance()
                ->setSubject("[CMHS-REAMS] " . $translator->trans("Confirmation of valid access to the Proethos2 platform"))
                ->setFrom($util->getConfiguration('committee.email'))
                ->setTo($user->getEmail())
                ->setBody(
                    $body
    ...
    public function listContactAction()
    {
        ...
        if($this->getRequest()->isMethod('POST')) {
            ...
            $message = \Swift_Message::newInstance()
            ->setSubject("[CMHS-REAMS] " . $translator->trans("Message from plataform."))
